...
class ApplyAcountPageObjdect(GG_BasePage):
    def __init__(self,driver):
        self.driver=driver
        self.item=self._get_elements_position('mycenter_apply_Account')

    def _get_elements_position(self,module):
        item = self.Read_elements_item(module)
        self.apply_accout_link =item.get('apply_account_link')
        self.modify_password_link=item.get('modify_password_link')
    ...
    def click_modify_password_link(self):
        find_type,value= self.get_type_locator(self.modify_password_link)
        self.wait_until_element(self.driver,self.according_type_switch_method(find_type,value)).click()

    #判断是否成功进入了企业申请账号页面

    def is_succesd_loadPage(self,str1):
